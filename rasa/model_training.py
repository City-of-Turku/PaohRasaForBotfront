...
async def train_async(
    ...
    agent_interpreter: Optional[Union[MultilingualNLUInterpreter, NaturalLanguageInterpreter]] = None,
) -> TrainingResult:
...
async def _do_training(
    ...
    if fingerprint_comparison_result.should_retrain_core():
        await _train_core_with_validated_data(
            ...
            fixed_model_name=fixed_model_name,
            additional_arguments=core_additional_arguments,
            interpreter=agent_interpreter or _load_interpreter(interpreter_path)
...
async def _train_nlu_with_validated_data(
    ...
    with ExitStack() as stack:
        ...
        async with telemetry.track_model_training(
            ...
            if file_importer.default_for_multilang_nlu_model:
                config = {file_importer.default_for_multilang_nlu_model : config[file_importer.default_for_multilang_nlu_model]}
            ...
            for lang in config:
                if config[lang]:
                    ...
                    if hasattr(agent_interpreter, 'interpreters') and lang in agent_interpreter.interpreters and hasattr(agent_interpreter.interpreters.get(lang), 'interpreter') and hasattr(agent_interpreter.interpreters.get(lang).interpreter, 'component_builder'):
                        component_builder = agent_interpreter.interpreters.get(lang).interpreter.component_builder
                    else:
