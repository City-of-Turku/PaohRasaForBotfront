...
async def _load_agent(
    ...
    after_training_load: bool = False,
) -> Agent:
    try:
        tracker_store = None
        generator = None
        ...
        loaded_agent = await rasa.core.agent.load_agent(
            ...
            interpreter=agent_interpreter,
            after_training_load=after_training_load,
        )
...
def create_app(
    ...
    async def train(request: Request, temporary_directory: Path) -> HTTPResponse:
        ...
        try:
            ...
            training_result = await train_async(**training_payload, agent_interpreter=app.agent.interpreter)
    ...
    async def _evaluate_model_using_test_set(
        ...
        evaluation = await rasa.nlu.test.run_evaluation(
            data_path,
            nlu_model.get(language) if language in nlu_model else list(nlu_model.values())[0],
            disable_plotting=True,
            errors=True,
            output_directory=model_directory,
        )
        if evaluation.get("entity_evaluation") is not None:
            ...
            # bf doesn't like evaluation without DIETClassifier key
            evaluation["entity_evaluation"] = { "DIETClassifier": None }
...
def _test_data_file_from_payload(
    request: Request, temporary_directory: Path, suffix: Text = ".md"
) -> Text:
    if request.headers.get("Content-type") == YAML_CONTENT_TYPE:
        return str(
