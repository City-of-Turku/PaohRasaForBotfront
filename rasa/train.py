...
async def _train_nlu_with_validated_data(
    ...
    with ExitStack() as stack:
        ...
        async with telemetry.track_model_training(
            ...
            if len(config) > 1:
                for lang in config:
                    if "default_lang" in config[lang] and config[lang]["default_lang"] == True:
                        config[lang]["language"] = [config[item]["language"] for item in config]
                        multilingual_config = {lang: config[lang]}
            if multilingual_config:
                config = multilingual_config

            logger.debug(f"Training new config: {config}")
