...
class Agent:
    ...
    def load(
        ...
        elif interpreter and isinstance(interpreter, MultilingualNLUInterpreter) and nlu_model and after_training_load:
            if hasattr(interpreter, 'interpreters') and hasattr(list(interpreter.interpreters.values())[0], 'interpreter') and hasattr(list(interpreter.interpreters.values())[0].interpreter, 'component_builder'):
                interpreter = rasa.core.interpreter.create_interpreter(nlu_model, component_builder=list(interpreter.interpreters.values())[0].interpreter.component_builder)
