...
def create_interpreter(
    ...
    # bf>
        ...
        return MultilingualNLUInterpreter(model_directory=obj, component_builder=component_builder)
    # </bf
    elif isinstance(obj, str) and os.path.exists(obj):
        return RasaNLUInterpreter(model_directory=obj, component_builder=component_builder)
...
class RasaNLUInterpreter(rasa.shared.nlu.interpreter.NaturalLanguageInterpreter):
    def __init__(
        ...
        if not lazy_init:
            self._load_interpreter(component_builder=component_builder)
        else:
    ...
    def _load_interpreter(self, component_builder: Optional[ComponentBuilder] = None,) -> None:
        from rasa.nlu.model import Interpreter
        self.interpreter = Interpreter.load(self.model_directory, component_builder=component_builder)
